...
KIND_CLUSTER ?= hiro-adaptive-orchestrator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name hiro-adaptive-orchestrator-builder
	$(CONTAINER_TOOL) buildx use hiro-adaptive-orchestrator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm hiro-adaptive-orchestrator-builder
	rm Dockerfile.cross
...
deploy-scheduler: manifests kustomize ## Deploy controller + webhook + cert-manager TLS (DEPLOY_SCHEDULER_PLUGIN=true).
	cd config/manager && "$(KUSTOMIZE)" edit set image controller=${IMG}
