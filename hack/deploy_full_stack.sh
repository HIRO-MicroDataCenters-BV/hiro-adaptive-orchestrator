...
print_external_prometheus_guide() {
  ...
  echo ""
  echo "  3. If it's a classic (non-Operator) Prometheus, add this job to its scrape_configs:"
  echo ""
  echo "       - job_name: 'hiro-adaptive-orchestrator'"
  ...
  echo "           - source_labels: [__meta_kubernetes_service_name]"
  echo "             action: keep"
  echo "             regex: ${NAME_PREFIX}controller-manager-metrics-service"
  echo "           - source_labels: [__meta_kubernetes_endpoint_port_name]"
  echo "             action: keep"
  echo "             regex: https"
  echo "         scheme: https"
  echo "         tls_config:"
  echo "           insecure_skip_verify: true"
  echo "         bearer_token_file: /var/run/secrets/kubernetes.io/serviceaccount/token"
  echo ""
  echo "     (bearer_token_file is a fixed path — Kubernetes auto-mounts a live token"
  echo "      there for whatever ServiceAccount that Prometheus pod itself runs as."
  echo "      Nothing to fetch or generate; just paste the line in as-is.)"
  echo ""
}
