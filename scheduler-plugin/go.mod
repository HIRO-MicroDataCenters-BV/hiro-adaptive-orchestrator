// Regenerate staging versions: hack/set-k8s-version.sh <k8s-version>
// Then run: cd scheduler-plugin && go mod tidy
//
// Building:
//   cd scheduler-plugin && go build -o ../bin/hiro-scheduler ./cmd/
//   (or use: make build-scheduler from repo root)
module github.com/HIRO-MicroDataCenters-BV/hiro-adaptive-orchestrator/scheduler-plugin

go 1.25.3

require (
	// Operator shared types (PlacementContext, DecisionResponse, NodeScore).
	// Pinned to the local tree via replace below — always in sync.
	github.com/HIRO-MicroDataCenters-BV/hiro-adaptive-orchestrator v0.0.0
	...
	// Scheduler framework — pins this binary to a specific K8s minor version.
	// Update with: hack/set-k8s-version.sh <new-version> && cd scheduler-plugin && go mod tidy
...
replace (
	// Always use the local operator tree so pkg/placement stays in sync.
	github.com/HIRO-MicroDataCenters-BV/hiro-adaptive-orchestrator => ../

	// Staging modules — versions must match k8s.io/kubernetes minor version.
	// Regenerated by hack/set-k8s-version.sh.
