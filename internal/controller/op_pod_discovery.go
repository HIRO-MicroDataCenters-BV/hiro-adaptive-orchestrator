...
//
// Delegates to utils.FindPodsForApplication, which is also used by the
// rebalance engine's trigger evaluators — kept in one place so both stay in
// sync on how a workload's pods are resolved.
func (r *OrchestrationProfileReconciler) findPodsForApplication(
	ctx context.Context,
	profile *orchestrationv1alpha1.OrchestrationProfile,
) ([]corev1.Pod, error) {
	return utils.FindPodsForApplication(ctx, r.Client, profile.Spec.ApplicationRef)
}
